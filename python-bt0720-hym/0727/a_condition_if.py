# 1.
# if문: 임의의 정수 3개를 입력받아 그 중에서 가장 큰 수를 출력하는 프로그램을 구현하세요
# 관계 연산자, 논리 연산자

# 사용자로부터 3개의 정수를 입력 바딕
...
# 2.
# if문: 미세먼지 저감 활동의 힐환으로 차량 2부제를 실시하고자 합니다
# 사용자로부터 차량번호를 입력받아 차량 번호가 짝수로 끝나면 '운행가능'
# , 아니면 '운행불가'를 출력하는 프로그램을 구현하세요.
# 단, 차량번호는 '237가 1234'와 같은 형식으로 입력받는다고 가정합니다.


# 사용자로부터 차량번호를 입력받기
car_number = input('차량번호를 입력해주세요(예 : 237가 1234) :')
